...
class UserController extends Controller
{
    ...
    public function update(UpdateRequest $request, User $user)
    {
        DB::transaction(function () use ($request, $user) {
            ...
            /** @var \Illuminate\Http\UploadedFile|null $imageFile */
            $imageFile = $request->file('image');
            if ($imageFile) {
                $fileName = "{$user->id}.{$imageFile->extension()}";
                $imageFile->storeAs('public/userImages', $fileName);
                $updateProperties['image_path'] = "/storage/userImages/{$fileName}";
            }

            $user->update($updateProperties);
        });
